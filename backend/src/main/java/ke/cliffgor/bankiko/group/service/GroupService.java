...
public class GroupService {
    ...
    public GroupResponse create(UUID userId, CreateGroupRequest request) {
        ...
        // Open a shared savings account in Fineract for the group pool
        ...
        SaccoGroup group = SaccoGroup.builder()
            ...
            .fineractGroupAccountId(savings.getResourceId())
            .createdBy(creator)
            .build();
    ...
    public Page<GroupResponse> listForUser(UUID userId, Pageable pageable) {
        return groupRepository.findGroupsByUserId(userId, pageable).map(this::toResponse);
    }
    ...
    private GroupResponse toResponse(SaccoGroup g) {
